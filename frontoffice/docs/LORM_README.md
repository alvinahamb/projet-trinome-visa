...
userModel.addDotEagerLoad("posts.comments");
List<User> users = userModel.all(connection);
```
...
## 5) Loading an Existing Model (load)

Use `load(connection)` when you already have a model instance and want to fetch its
current state from the database. You must set the primary key first, then call `load`.

`load` uses `find()` under the hood, but it **does not overwrite your current in-memory
fields**. It only stores the database state internally (for checks like “exists?”),
while your current object values remain unchanged.

```java
// Example: load an existing vehicle by id
Vehicule vehicule = new Vehicule();
vehicule.setIdVehicule(10); // primary key must be set
vehicule.load(connection);
```

Typical use case: ensure a record exists before an update.

```java
Vehicule vehicule = new Vehicule();
// Do modifications to the vehicule object here (set other fields)...
vehicule.setIdVehicule(10);
// ...

vehicule.load(connection); // throws ModelNotFoundException if not found
vehicule.update(connection);
```

This is the most common flow when updating a model.

⚠️ Important: if you do `new Vehicule()` and only set the id and one column while leaving
the others `null`, then calling `update` will keep those other columns as `null` as well.
Calling `load` does not change that — it will NOT fill missing fields from the database.

In short: `load` checks existence and stores DB state internally, but it does not
mutate your current object state.

## 6) Transforming Values: beforeIn / beforeOut

You can override `beforeIn()` and `beforeOut()` to transform column values when reading from or writing to the database. Both methods return a map of column names to transformation functions.

- `beforeOut()` runs **before persisting** values to the database.
- `beforeIn()` runs **after reading** values from the database.

Example using `Reservation` to convert `LocalDateTime` to `Timestamp` and back:

```java
@Override
public Map<String, Function<Object, ?>> beforeOut() {
    return Map.of(
        "date_heure_arrive", value -> Timestamp.valueOf((LocalDateTime) value)
    );
}
...
public Map<String, Function<Object, ?>> beforeIn() {
    return Map.of(
        "date_heure_arrive", value -> ((Timestamp) value).toLocalDateTime()
    );
}
```
